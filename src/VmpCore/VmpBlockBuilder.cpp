...
bool VmpBlockBuilder::executeVmPopReg(VmpNode& nodeInput, VmpOpPopReg* inst)
{
	if (buildCtx->vmreg.isSelected) {
		return true;
	}
	buildCtx->vmreg.reg_code = inst->reg_code;
	buildCtx->vmreg.reg_stack = inst->reg_stack;
...
bool VmpBlockBuilder::executeVmpOp(VmpNode& nodeInput,std::unique_ptr<VmpInstruction> inst)
{
	...
	case VM_INIT:
		...
		break;
	case VM_POP_REG:
		executeVmPopReg(nodeInput, (VmpOpPopReg*)vmInst);
		break;
	case VM_JMP:
...
bool VmpBlockBuilder::tryMatch_vShrd(ghidra::Funcdata* fd, VmpNode& nodeInput)
{
	...
	if (asmData->raw->id == X86_INS_SHRD) {
		...
		vOpShld->addr = nodeInput.readVmAddress(buildCtx->vmreg.reg_code);
		executeVmpOp(nodeInput, std::move(vOpShld));
		return true;
	}
	return false;
}
...
std::unique_ptr<VmpInstruction> VmpBlockBuilder::tryMatch_vPopReg(ghidra::Funcdata* fd, VmpNode& nodeInput)
{
	...
	vPopRegOp->loadAddr = srcResult[0].addr;
...
bool VmpBlockBuilder::Execute_FINISH_VM_INIT()
{
	...
	if (it != cache.handlerPatternMap.end()) {
		std::unique_ptr<VmpInstruction> vmInstruction = it->second->MakeInstruction(nodeInput);
	...
	if (newVmPattern != nullptr) {
		std::unique_ptr<VmpInstruction> vmInstruction = newVmPattern->MakeInstruction(nodeInput);
