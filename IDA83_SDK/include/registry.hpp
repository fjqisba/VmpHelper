...
// note: root_name[] in register.cpp
...
idaman THREAD_SAFE bool ida_export reg_bin_op(
        const char *name,
        bool save,
        void *data,
        size_t datalen,
        const char *subkey,
        int mode = 0);
idaman THREAD_SAFE bool ida_export reg_str_get(qstring *buf, const char *name, const char *subkey);
idaman THREAD_SAFE void ida_export reg_str_set(const char *name, const char *subkey, const char *buf);
idaman THREAD_SAFE int ida_export reg_int_op(
        const char *name,
        bool save,
        int value,
...
idaman THREAD_SAFE void ida_export reg_update_strlist(
        const char *subkey,
        const char *add,
        size_t maxrecs,
...
/// \param name     value name
/// \param data     input, must not be nullptr
/// \param datalen  length of input in bytes
/// \param subkey   key name

inline THREAD_SAFE void reg_write_binary(
        const char *name,
        const void *data,
        size_t datalen,
        const char *subkey = nullptr)
...
inline THREAD_SAFE void reg_write_binary(
        ...
        const char *subkey = nullptr)
{
  return reg_write_binary(name, data.begin(), data.size(), subkey);
}


/// Read binary data from the registry.
/// \param name       value name
/// \param[out] data  result, must not be nullptr
...
inline THREAD_SAFE bool reg_read_binary(
        const char *name,
        void *data,
        size_t datalen,
        const char *subkey = nullptr)
...
inline THREAD_SAFE bool reg_read_binary_part(
        const char *name,
        void *data,
        size_t datalen,
        const char *subkey = nullptr)
...
/// Read binary data from the registry.
/// \param name       value name
/// \param[out] data  output buffer, must not be nullptr
/// \param subkey     key name
/// \return success
...
inline THREAD_SAFE void reg_write_string(
        const char *name,
        const char *utf8,
        const char *subkey = nullptr)
{
...
inline THREAD_SAFE bool reg_read_string(
        qstring *utf8,
        const char *name,
        const char *subkey = nullptr)
{
...
inline THREAD_SAFE int reg_read_int(const char *name, int defval, const char *subkey = nullptr)
{
...
inline THREAD_SAFE void reg_write_int(const char *name, int value, const char *subkey = nullptr)
{
...
inline THREAD_SAFE bool reg_read_bool(const char *name, bool defval, const char *subkey = nullptr)
{
...
inline THREAD_SAFE void reg_write_bool(const char *name, int value, const char *subkey = nullptr)
{
...
inline THREAD_SAFE bool reg_subkey_subkeys(qstrvec_t *out, const char *name)
{
...
inline THREAD_SAFE bool reg_subkey_values(qstrvec_t *out, const char *name)
{
...
inline THREAD_SAFE void reg_update_filestrlist(
        const char *subkey,
        const char *add,
        size_t maxrecs,
        const char *rem = nullptr)
...
inline THREAD_SAFE void regset_ ## func(bool value)       \
  { reg_write_bool(_RVN_(func), value); }     \
inline THREAD_SAFE bool regget_ ## func(bool def)         \
  { return reg_read_bool(_RVN_(func), def); }

#define REG_INT_FUNC(func, valname)           \
REG_VAL_NAME(func,valname);                   \
inline THREAD_SAFE void regset_ ## func(int value)        \
{                                             \
  reg_int_op(_RVN_(func), true, value);       \
}                                             \
inline THREAD_SAFE int regget_ ## func(int def=0)         \
{                                             \
  return reg_int_op(_RVN_(func), false, def); \
}

/// Define the Windows registry subkey (under HKCU\\Software\\Hex-Rays)
/// acting as the root of the IDA registry. Under Linux/Mac, it is used as
/// a filename with extension ".cfg". Defaults to "IDA" (lowercase in Linux/Mac).
idaman THREAD_SAFE bool ida_export set_registry_root(const char *name);
...
inline THREAD_SAFE void regget_history(qstrvec_t *list)
{
#ifdef DEMO
  qnotused(list);
#else
...
inline THREAD_SAFE void reg_update_history(const char *addfile, const char *removefile = nullptr)
{
#ifdef DEMO
  qnotused(addfile);
  qnotused(removefile);
...
inline THREAD_SAFE void reg_history_size_truncate(void)
{
#ifndef DEMO
...
idaman DEPRECATED void ida_export reg_load(void);
idaman DEPRECATED void ida_export reg_flush(void);
