...
void ActionDatabase::buildVmpBlockAction(Architecture* conf)
{
    ...
    act->addAction(actstackstall);

	act->addAction(new ActionRedundBranch("deadcontrolflow")); // dead code removal
    act->addAction(new ActionBlockStructure("blockrecovery"));
    act->addAction(new ActionConstantPtr("typerecovery"));
	{
    ...
    act->addAction(actprop2);

    act->addAction(new ActionNodeJoin("nodejoin"));
    act->addAction(new ActionConditionalExe("conditionalexe"));
    act->addAction(new ActionConditionalConst("analysis"));
    act->addAction(new ActionLikelyTrash("protorecovery"));
    act->addAction(new ActionDirectWrite("protorecovery_a", true));
    act->addAction(new ActionDirectWrite("protorecovery_b", false));
    act->addAction(new ActionVmpHandlerDeadCode("deadcode", stackspace));
    act->addAction(new ActionDoNothing("deadcontrolflow"));
    act->addAction(new ActionSwitchNorm("switchnorm"));
    act->addAction(new ActionReturnSplit("returnsplit"));
    act->addAction(new ActionUnjustifiedParams("protorecovery"));
	act->addAction(new ActionStartTypes("typerecovery"));
